# -*- coding: utf-8 -*-
"""
This file contains the Qudi module for the confocal scanner.
...
This file works off of the scanning_probe_dummy.py example, to implement control of a mcl piezo stage
"""
...
from qudi.hardware.piezo_scanner.mcl import MCL_Madlib_Wrapper as madlib
...
class PiezoScanConstraints(ScanConstraints):
...
class PiezoScannerInterfuseBare(ScanningProbeInterface):
    """
    Scanning probe microscope. Produces a picture with several gaussian spots.

    Example config for copy-paste:

    piezo_scanner_interfuse:
        module.Class: 'piezo_scanner.piezo_scanner_mcl.PiezoScannerInterfuse'
        connect:
            #scan_hardware: 'piezo_scanner'
        ...
        options:
            ...
            position_feedback_available: True # optional
    ...
    _counter_hardware = Connector(name='counter_hardware', interface='FastCounterInterface')

    _position_axes: List[str] = ConfigOption(name='position_axes', missing='error')
    _min_response_time: Dict[str, int] = ConfigOption(name='min_response_time', missing='error')
    _resolution_ranges: Dict[str, List[float]] = ConfigOption(name='resolution_ranges', missing='error')
    _serial_number: int = ConfigOption(name='serial_number', missing='error')


    _input_channel_units: Dict[str, str] = ConfigOption(name='input_channel_units', missing='error')

    # config options for image generation
    _max_spot_number: int = ConfigOption(name="max_spot_number", default=int(80e3), constructor=lambda x: int(x))
    _spot_density: float = ConfigOption(name="spot_density", default=2e5)  # in 1/m
    ...
    _position_feedback_available: bool = ConfigOption(name='position_feedback_available', default=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._position_ranges = None
        self._dac_resolution = 16
        self._position_accuracy = 10**-10
        ...
        self._back_scan_vectors = None
        self.__cur_forward_pixel = 0
        self.__cur_backward_pixel = 0
        self.__cur_pixel = 0
        self.__prev_time = None
        self._min_timer_interval_ms = 2
        self._pos_moving = True
        self._aq_lines = 0
        self._is_finished = False

        self._axis_to_ind_mcl = {'x':1, 'y':2, 'z':3} # MCL Nanodrive expects 'x' axis to start at index 1

        self._image_generator: Optional[ImageGenerator] = None
        # "Hardware" constraints
        self._constraints: Optional[PiezoScanConstraints] = None
    ...
    def on_activate(self):
        ...
        # connect to the MCL Nanodrive
        piezo_manager = madlib.MCL_Nanodrive()
        piezo_manager.grab_all_handles()
        # TODO allow a unique handle for each axis. User may be using multiple stages to control all three axes
        self._handle = piezo_manager.get_handle_by_serial(self._serial_number)
        self._manager = piezo_manager
        self.log.info(
            f"MadCityLabs NanoDrive s/n "
            f"{self._serial_number}"
            f" is connected through handle "
            f"{self._handle}."
        )
        self._product_info = self._manager.get_product_info(self._handle) # TODO we could get the axis info, but I think it is better to have user input that in config

        self._position_ranges = {ax: [0, math.floor(self._manager.get_calibration(self._axis_to_ind_mcl[ax], self._handle))*10**-6] for ax in self._position_axes}
        self._dac_resolution = 2**self._product_info["dac_resolution"]
        self._position_accuracy = {ax: self._position_ranges[ax][1] / self._dac_resolution for ax in self._position_axes }
        ...
        for ii, (axis, ax_range) in enumerate(self._position_ranges.items()):
            ...
            resolution_range = tuple(self._resolution_ranges[axis]) #defines the resolution constraints
            res_default = min(resolution_range[1], 100) # defines the default start value in the gui
            frequency_range = tuple([1, 1/self._min_response_time[axis]]) #converts response time to frequency range
            f_default = min(frequency_range[1]/5, 1e2) # defines the default start value in the gui
        ...
        #channels = [ #define different data channels. Not sure how this will connect to an actual device yet
        ...
        for channel, unit in self._input_channel_units.items():
            if unit == 'cps':
                conversion_factor = 1 / self._counter_hardware()._gated_readout_s_scanned_measurement #TODO would like to make this smoother... somehow include that in the cfg
            else:
        ...
        self.log.info(f"counter status {self._counter_hardware().get_status()}")
        ...
        self._min_position = {ax.name: np.min(ax.position.bounds) for ax in self.constraints.axes.values()}  # start the positioner at minimum range
        self._current_position = {ax.name: np.min(ax.position.bounds) for ax in self.constraints.axes.values()}  # start the positioner at minimum range
        ...
        # Create fixed maps of spots for each scan axes configuration. I think, in the actual file, I will get rid of these?
    ...
    def on_deactivate(self):
        ...
        # move to the minimum position on each axis

        if self.module_state() != 'idle':
            raise RuntimeError('Stage is not idle. Unable to move to position.')
        else:
            self.move_absolute(self._min_position)  # try moving the stage
            self.log.info(f'Stage is moved to current position {self._current_position} of min position {self._min_position}')
            self._current_position = self._min_position

        self._manager.release_handle(self._handle)
        self.log.info(
            f"MadCityLabs NanoDrive s/n "
            f"{self._serial_number}"
            f" is disconnected from handle "
            f"{self._handle}."
        )
    ...
    def configure_scan(self, settings: ScanSettings) -> None:
        ...
        """
        with self._thread_lock:
            self.log.debug('Scanning probe "configure_scan" called.')
            # Sanity checking
            if self.module_state() != 'idle':
                raise RuntimeError(
            ...
            #self.log.info(f'{settings} frequency is {settings.frequency}')
    ...
    def move_absolute(self, position, velocity = None, blocking = False):
        ...
        """
        with self._thread_lock:
            # self.log.debug('Scanning probe "move_absolute" called.')
            #if self.module_state() != 'idle':
            ...
            if not set(position).issubset(self._position_ranges):
                ...
                for ax in position:
                    self._manager.single_write_n(position[ax]*10**6,
                                                 self._axis_to_ind_mcl[ax],
                                                 self._handle)
    ...
    def move_relative(self, distance, velocity = None, blocking = False):
        ...
        """
        with self._thread_lock:
            self.log.debug('Scanning probe "move_relative" called.')
            #if self.module_state() != 'idle':
            ...
            if not set(distance).issubset(self._position_ranges):
                ...
                for ax in new_pos:
                    self._manager.single_write_n(new_pos[ax]*10**6,
                                                 self._axis_to_ind_mcl[ax],
                                                 self._handle)
    ...
    def get_position(self): #this is important too,this will be the read() command
        """
        ...
        """
        with self._thread_lock:
            #self.log.debug('Piezo scanner "get_position" called.')
            current_position_read = {}
            for ax in self._scan_data.settings.position_feedback_axes:
                current_position_read[ax] = self._manager.single_read_n(self._axis_to_ind_mcl[ax], self._handle)*10**-6
            return current_position_read
    ...
    def start_scan(self):
        ...
        """
        with self._thread_lock:
            self.log.debug('Piezo scanner "start_scan" called.')
            if self.module_state() != 'idle':
            ...
            self._scan_vectors = self._init_scan_vectors() # get scan vectors

            self.module_state.lock()
            # generate the fake scan data
            self._scan_image = self._image_generator.generate_image(self._scan_vectors, self.scan_settings.resolution)

            # create class for the scan data
            ...
            # initiate a new scan through scan_data
            self._scan_data.new_scan()

            #bin_width_s, record_length_s, read_window_s, number_of_gates=0
            self._counter_readout_dur = self._counter_hardware()._gated_readout_s_scanned_measurement
            number_of_gates = 1
            if 1/self.scan_settings.frequency < self._counter_readout_dur:
                raise ValueError(f'Scan frequency ({self.scan_settings.frequency} Hz / dwell time {1000/self.scan_settings.frequency} ms) is too fast for set readout duration {self._counter_readout_dur *1000} ms')
            self._counter_hardware().configure(self._counter_readout_dur, number_of_gates)
            self._counter_hardware().start_tiger()

            # If back scan enabled, follow similar steps
            if self._back_scan_settings is not None:
                self._back_scan_vectors = self._init_back_scan_vectors() # get scan vectors

                self._back_scan_image = self._image_generator.generate_image(
                    self._back_scan_vectors, self.back_scan_settings.resolution
                )
                ...
                self.__cur_backward_pixel = self.back_scan_settings.resolution[0]

            # Prepare values for stepping through scan
            self._aq_lines = 0
            self._is_finished = False
            self._pos_moving = True
            ...
            start_pos = {ax: self._scan_vectors[ax][self.__cur_pixel] for ax in
                       self._scan_vectors}
            self.move_absolute(start_pos)

            # Set the refresh rate for checking the scan
            timer_interval_ms = min(1 / self.scan_settings.frequency * 1000, self._min_timer_interval_ms)
            self.__update_timer.setInterval(timer_interval_ms)

        # Start the timer for the refresh rate, and start iterating through scan
        self.__start_timer()
    ...
    def stop_scan(self):
        ...
        self.log.debug('Scanning probe "stop_scan" called.')
        if self.module_state() == 'locked':
            with self._thread_lock:
                self._scan_image = None
                self._back_scan_image = None
    ...
    def __update_scan_data(self) -> None:
        ...
        cur_time = time.time()
        t_elapsed = cur_time - self.__prev_time

        # Step through each position. pos_moving defines which direction the scan is iterating
        if self._pos_moving:
            if t_elapsed >= 1/self.scan_settings.frequency:
                self._counter_hardware().start_measure()

                # read sensor of position feedback axes, if available
                if self._position_feedback_available:
                    cur_pos_read = self.get_position()
                    for ax in self._scan_data.settings.position_feedback_axes:
                        self._scan_data.position_data[ax].flat[self.__cur_pixel] = cur_pos_read[ax]
                # get fake data from generated scan

                new_data = self._counter_hardware().read_counts(1)
                #new_forward_data = self._scan_image.T.flat[self.__cur_pixel]  # this is where we would get actual data

                # Update data channels with new data
                for ch in self.constraints.channels:
                    self._scan_data.data[ch].T.flat[ self.__cur_pixel] = new_data[0] * self.constraints.channels[ch].conversion

                self._counter_hardware().stop_measure()

                # Iterate to the next step, depending on if we are at end of a row or not
                if ((self.__cur_pixel + 1) % self.scan_settings.resolution[0]) == 0:
                    self._pos_moving = False
                    self.__cur_backward_pixel -= 1
                    self.__cur_pixel = self.__cur_backward_pixel

                    # set time interval based on backscan frequency
                    if self._back_scan_settings is not None:
                        timer_interval_ms = min(1 / self.back_scan_settings.frequency * 1000, self._min_timer_interval_ms)
                        self.__update_timer.setInterval(timer_interval_ms)
                    else:
                        # If no backscan, just move to next point
                        self._pos_moving = True
                        self.__cur_forward_pixel += 1
                        self.__cur_pixel = self.__cur_forward_pixel
                        self._aq_lines += 1

                        # Check conditions for ending the scan
                        if self.scan_settings.scan_dimension == 1:
                            self._is_finished = self._aq_lines >= 1
                        else:
                            self._is_finished = self._aq_lines >= self.scan_settings.resolution[1]
                else:
                    self.__cur_forward_pixel += 1
                    self.__cur_pixel = self.__cur_forward_pixel
                ...
                if not self._is_finished:
                    ...
                    new_pos = {ax: self._scan_vectors[ax][self.__cur_pixel] for ax in
                               self._scan_vectors}
            ...
            # step through a backscan, if available
            if t_elapsed >= 1/self.back_scan_settings.frequency:
                self._counter_hardware().start_measure()

                # read sensor of position feedback axes, if available
                cur_pos_read = self.get_position()
                for ax in self._scan_data.settings.position_feedback_axes:
                    self._back_scan_data.position_data[ax].flat[self.__cur_pixel] = cur_pos_read[ax]

                new_data = self._counter_hardware().read_counts(1)

                # get fake data from generated scan
                #new_forward_data = self._back_scan_image.T.flat[self.__cur_pixel]  # this is where we would get actual data
                # Update data channels with new data
                for ch in self.constraints.channels:
                    self._back_scan_data.data[ch].T.flat[self.__cur_pixel] = new_data[0] * self.constraints.channels[ch].conversion

                self._counter_hardware().stop_measure()

                # Iterate to the next step, depending on if we are at beginning of a row or not
                if (self.__cur_pixel % self.back_scan_settings.resolution[0]) == 0:
                    self._pos_moving = True
                    self.__cur_forward_pixel += 1
                    self.__cur_pixel = self.__cur_forward_pixel
                    self._aq_lines += 1
                    self.__cur_backward_pixel =  (self._aq_lines + 1) * self.back_scan_settings.resolution[0]

                    # set time interval based on forward scan frequency
                    timer_interval_ms = min(1 / self.scan_settings.frequency * 1000, self._min_timer_interval_ms)
                    self.__update_timer.setInterval(timer_interval_ms)
                else:
                    self.__cur_backward_pixel -= 1
                ...
                if not self._is_finished:
                    # Move to the new position
                    new_pos = {ax: self._back_scan_vectors[ax][self.__cur_pixel] for ax in
                               self._back_scan_vectors}
                    self.move_absolute(new_pos)
        ...
        if self._is_finished:
            self.module_state.unlock()
            self._counter_hardware().stop_tiger()
    ...
    def _init_scan_vectors_from_scan_settings(self) -> Dict[str, np.ndarray]:
        ...
        meshgrids = np.meshgrid(*axes_scan_values, indexing="xy") # originally 'ij', but this would swap x<->y in 2D scans. This fixes that
    ...
    def _init_back_scan_vectors_from_back_scan_settings(self) -> Dict[str, np.ndarray]:
        axes_scan_values = [
            np.linspace(
                self.back_scan_settings.range[ii][0], self.back_scan_settings.range[ii][1], self.back_scan_settings.resolution[ii]
            )
            for ii, _ in enumerate(self.back_scan_settings.axes)
        ]
        # generate all combinations of points
        meshgrids = np.meshgrid(*axes_scan_values, indexing="xy") # originally 'ij', but this would swap x<->y in 2D scans. This fixes that
        # create position vector dictionary by raveling the grids
        back_scan_vectors = {axis: grid.ravel() for axis, grid in zip(self.back_scan_settings.axes, meshgrids)}
        return back_scan_vectors

    def _init_back_scan_vectors(self) -> Dict[str, np.ndarray]:
        back_scan_vectors = self._init_back_scan_vectors_from_back_scan_settings()
        back_scan_vectors = self._expand_coordinate(back_scan_vectors)  # always expand to all scan dims
...
class PiezoScannerInterfuse(CoordinateTransformMixin, PiezoScannerInterfuseBare):
    def _init_scan_vectors(self) -> Dict[str, np.ndarray]:
    ...
    def _init_back_scan_vectors(self) -> Dict[str, np.ndarray]:
        back_scan_vectors = super()._init_back_scan_vectors()

        if self.coordinate_transform_enabled:
            return self.coordinate_transform(back_scan_vectors)
