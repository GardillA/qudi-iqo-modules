...
class PiezoScannerBare(ScanningProbeInterface):
    """
    ...
    piezo_scanner:
        module.Class: 'piezo_scanner.piezo_scanner_mcl.PiezoScanner'
        options:
            position_axes: [x, y, z]
            min_response_time:
                x: 10e-3
        ...
        if self._pos_moving:
            if t_elapsed >= 1/self.scan_settings.frequency:
                ...
                for ch in self.constraints.channels:
                    self._scan_data.data[ch].T.flat[ self.__cur_pixel] = new_forward_data
            ...
            if t_elapsed >= 1/self.back_scan_settings.frequency:
                ...
                # Update data channels with new data
                for ch in self.constraints.channels:
                    self._back_scan_data.data[ch].T.flat[self.__cur_pixel] = new_forward_data  #
...
class PiezoScanner(CoordinateTransformMixin, PiezoScannerBare):
    def _init_scan_vectors(self) -> Dict[str, np.ndarray]:
